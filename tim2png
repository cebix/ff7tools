...
def readTimImage(f):
    ...
    if haveClut:
        ...
        if pMode == 0:
            clut += b'\0' * 32 * 16  # extend to 256 entries
            clut = clut[:512]
    ...
    if pMode == 0:
        ...
        image = Image.frombytes("P", (width, height), pixelData, "raw", "P", 0, 1)
        image.palette = palette
        ...
        image = Image.frombytes("RGB", (width, height), output, "raw", "BGR;15", 0, 1)
        ...
        image = Image.frombytes("RGB", (width, height), pixelData, "raw", "RGB", 0, 1)
